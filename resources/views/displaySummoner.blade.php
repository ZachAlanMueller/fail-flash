        ...
        <div class="tab-content">
            ...
            <div class="tab-pane fade" id="champions-pills">
							<div class="row">
								<div class="col-xs-6">
									@if(true)
										<canvas id="topChamps" width="auto" height="auto"></canvas>
										<script>
											$(document).ready(function() {
												var ctx = document.getElementById('topChamps');
												var topChamps = new Chart(ctx, {
										type: 'horizontalBar',
										data: {
									    labels: [
									        @foreach($champions->topTen as $a => $champion)
													"{{$champion->name}}"
													@if($a != count($champions->topTen)), @endif
													@endforeach
									    ],
									    datasets: [
									        {
